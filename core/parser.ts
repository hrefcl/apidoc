    ...
    for (let i = 0; i < blocks.length; i += 1) {
        ...
        for (let j = 0; j < blocks[i].length; j += 1) {
            ...
            if (
                ...
                blocks[i][j].name.substr(0, 4) === 'code' ||
                // JSDoc/TSDoc tags
                [
                    'file',
                    'author',
...
Parser.prototype.findElements = function (block, filename) {
    ...
    // Elements start with @api, @openapi, @mqtt, @iot, @model, or @code (at beginning of comment line, not in text)
    const elementsRegExp =
        /((?:^|\uffff)[\s*]*@(api\w*|openapi(?:-\w+)?|mqtt\w*|iot\w*|model\w*|code\w*|payloadSchema|examplePublish|exampleSubscribe|responseTopic|responseExample|topicParam|topic|payload|qos|retain|author|ratelimit|errors|tags|auth|file|copyright|license|package|see|param|returns|remarks|example|public|internal|alpha|beta)(?:\s*([\s\S]*?))?(?=\uffff[\s*]*@(?:api\w*|openapi|mqtt\w*|iot\w*|model\w*|code\w*|payloadSchema|examplePublish|exampleSubscribe|responseTopic|responseExample|topicParam|topic|payload|qos|retain|author|ratelimit|errors|tags|auth|file|copyright|license|package|see|param|returns|remarks|example|public|internal|alpha|beta)|\*\/|$))/gm;
