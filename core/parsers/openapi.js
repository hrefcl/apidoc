 ...
 */
function processOpenApiForApiDoc(openapi, rawContent) {
 ...
 */
function extractFirstOperation(paths, components) {
 ...
 */
function processStandaloneOperation(operation) {
    const result = {
 ...
 */
function createDocumentationEntry(openapi) {
    const result = {
        type: 'openapi-doc',
 ...
 */
function extractVersionFromOpenAPI(openapi) {
 ...
 */
function isSchemaDefinition(obj) {
    if (!obj || typeof obj !== 'object')
        return false;
 ...
 */
function isOperationDefinition(obj) {
    if (!obj || typeof obj !== 'object')
        return false;
 ...
 */
function convertParametersToApiDoc(parameters) {
 ...
 */
function convertRequestBodyToApiDoc(requestBody, components) {
    const bodyFields = [];
 ...
 */
function convertResponsesToApiDoc(responses, components) {
    const success = [];
    const error = [];
 ...
 */
function extractSchemaFields(schema, contentType, statusCode, components) {
    const fields = [];
 ...
 */
function resolveSchemaReference(schema, components) {
 ...
 */
function parseExternalOpenApiFile(pathSpec, filePath, currentFilePath) {
    try {
 ...
 */
function extractSpecificOperationFromFile(openapi, pathSpec) {
 ...
 */
function convertOpenApiToElements(openapi, sourceFile, pathSpec) {
    const elements = [];
    if (!openapi.paths) {
        return elements;
    }
 ...
 */
function externalFileProcessor(elements, element, block, filename) {
