...
/**
 * Parses @apiSchema tag content
 *
 * @description This function is a placeholder. The actual processing happens
 * in the processor hook which is registered via init().
 *
 ...
 *
 * @returns Empty object - actual processing done by processor hook
 *
 * @internal
 * @since 4.0.0
 */
function parse(content: string, source: string): any {
    // @apiSchema processing happens in the processor hook
    // Return empty object to satisfy APIDoc
    return {};
}

/**
 * Internal parsing function used by the processor hook
 *
 * @description Extracts schema configuration from @apiSchema tag content.
 * Supports both TypeScript interface references and external JSON Schema files.
 *
 * @param content - The raw content string from the @apiSchema tag
 * @param source - The complete source line (used for error reporting)
 *
 ...
 * parseInternal("(Body) {interface=UserProfile} apiParam", "@apiSchema ...")
 * // Returns: { group: "Body", schemaType: "interface", schemaValue: "UserProfile", element: "apiParam" }
 *
 * // JSON Schema for success response
 * parseInternal("{jsonschema=./schemas/user.json} apiSuccess", "@apiSchema ...")
 ...
 * @since 4.0.0
 */
function parseInternal(content: string, source: string): any {
...
export default {
    parse,
    path: 'local',
    method: 'insert', // Use insert (extend) instead of push to avoid array issues
    markdownFields: [],
