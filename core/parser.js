...
const find_files_1 = __importDefault(require("./utils/find_files"));
 ...
 * Inherit
 */
util_1.default.inherits(Parser, Object);
/**
...
Parser.prototype.parseFiles = function (options, parsedFiles, parsedFilenames) {
    const self = this;
    find_files_1.default.setPath(options.src);
    find_files_1.default.setExcludeFilters(options.excludeFilters);
    find_files_1.default.setIncludeFilters(options.includeFilters);
    const files = find_files_1.default.search();
    // Parser
...
Parser.prototype.parseFile = function (filename, encoding) {
    ...
    self.extension = path_1.default.extname(filename).toLowerCase();
    ...
    const fileContent = fs_1.default.readFileSync(filename, { encoding: 'binary' });
...
Parser.prototype.parseSource = function (fileContent, encoding, filename) {
    ...
    self.src = iconv_lite_1.default.decode(buffer, encoding);
        ...
        for (let j = 0; j < elements.length; j += 1) {
            ...
            if (!elementParser) {
                ...
                try {
                    ...
                    if (!pathTo) {
                        throw new parser_error_1.ParserError('pathTo is not defined in the parser file.', '', '', element.sourceName, '');
                    }
                    ...
                    if (attachMethod !== 'insert' && attachMethod !== 'push') {
                        throw new parser_error_1.ParserError('Only push or insert are allowed parser method values.', '', '', element.sourceName, '');
                    }
                    ...
                    if (e instanceof parameter_error_1.ParameterError) {
                        const extra = [];
                        if (e.definition) {
                        ...
                        throw new parser_error_1.ParserError(e.message, self.filename, blockIndex + 1, element.sourceName, element.source, extra);
                    }
                    throw new parser_error_1.ParserError('Undefined error.', self.filename, blockIndex + 1, element.sourceName, element.source);
                }
                if (!values) {
                    throw new parser_error_1.ParserError('Empty parser result.', self.filename, blockIndex + 1, element.sourceName, element.source);
                }
                if (preventGlobal) {
                    ...
                    if (Object.keys(blockData.global).length > countAllowedMultiple) {
                        throw new parser_error_1.ParserError('Only one definition or usage is allowed in the same block.', self.filename, blockIndex + 1, element.sourceName, element.source);
                    }
                }
                ...
                if (pathTo === 'global' || pathTo.substr(0, 7) === 'global.') {
                    if (allowMultiple) {
                        ...
                        if (Object.keys(blockData.global).length > 0) {
                            throw new parser_error_1.ParserError('Only one definition is allowed in the same block.', self.filename, blockIndex + 1, element.sourceName, element.source);
                        }
                        if (preventGlobal) {
                            throw new parser_error_1.ParserError('Only one definition or usage is allowed in the same block.', self.filename, blockIndex + 1, element.sourceName, element.source);
                        }
    ...
    for (let i = 0; i < blocks.length; i += 1) {
        ...
        for (let j = 0; j < blocks[i].length; j += 1) {
            ...
            if (filterTag) {
                ...
                if (blocks[i][j].name.substr(0, filterTag.length) === filterTag &&
