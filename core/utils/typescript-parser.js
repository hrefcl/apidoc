...
exports.extractInterfacesFromFile = extractInterfacesFromFile;
exports.findInterface = findInterface;
exports.interfaceToApiDocElements = interfaceToApiDocElements;
 ...
 * @since 4.0.0
 */
function findInterface(interfaceName, currentFile) {
    // Check cache first
    const cached = interfaceCache.get(`${currentFile}:${interfaceName}`);
    if (cached)
        return cached;
    // Get the directory to search in
    const dir = path_1.default.dirname(currentFile);
    // Try to find in TypeScript files in the same directory
    try {
        const files = fs_1.default.readdirSync(dir).filter((f) => f.endsWith('.ts') || f.endsWith('.tsx'));
 ...
 */
function normalizeTypeScript(type) {
 ...
 * @since 4.0.0
 */
function interfaceToApiDocElements(iface, elementType = 'apiParam', group) {
    const elements = [];
