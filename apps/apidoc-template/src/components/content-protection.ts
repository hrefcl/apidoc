...
class ContentProtection {
  ...
  static protectSensitiveSections(html: string, password: string): { protectedHtml: string; encryptedSections: EncryptedContent[] } {
    ...
    const encryptedSections = sensitiveSections.map(section =>
      this.encryptContent(section.content, password)
    );
   ...
   */
  static restoreProtectedContent(
    protectedHtml: string,
    encryptedSections: EncryptedContent[],
    password: string
  ): string {
    ...
    try {
      ...
      encryptedSections.forEach((encryptedSection, index) => {
        ...
        const markerRegex = new RegExp(
          `${this.CONTENT_MARKER}[\\s\\S]*?${this.CONTENT_MARKER_END}`,
          'g'
        );
      ...
      restoredHtml = restoredHtml.replace(
        /data-protected="true"\s+style="display:\s*none;"/g,
        'style="display: block;"'
      );
...
export { EncryptedContent, ContentSection };