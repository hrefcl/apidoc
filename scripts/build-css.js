...
async function buildCSS() {
    try {
        ...
        try {
            const tailwindCommand = ['npx', '@tailwindcss/cli@next', '--input', cssInputFile, '--output', outputFile, '--config', path.join(__dirname, '..', 'tailwind.config.js')];
        ...
        if (!success) {
            ...
            try {
                const fallbackCommand = ['npx', 'tailwindcss', '--input', cssInputFile, '--output', outputFile, '--config', path.join(__dirname, '..', 'tailwind.config.js')];
        ...
        if (!success) {
            ...
            try {
                ...
                const result = await postcss([tailwindcssPostcss, autoprefixer]).process(css, {
                    from: cssInputFile,
                    to: outputFile,
